...
import CaseStudyRole from '@/components/case-study/case-study-role.astro';
import CaseStudyTags from '@/components/case-study/case-study-tags.astro';
import CaseStudyTeam from '@/components/case-study/case-study-team.astro';
import ReviewCard from '@/components/case-study/review-card.astro';
import SectionDark from '@/components/case-study/section-dark.astro';
...
Les frustrations identifiées reflètent l'insatisfaction des utilisateurs avant l'amélioration de l'application.

<div class="not-prose grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
  <div class="flex flex-col gap-3">
    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-brand-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="7" height="7" x="14" y="3" rx="1"/><path d="M10 21V8a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-5a1 1 0 0 0-1-1H3"/></svg>
    <p class="text-sm text-hero-foreground/80">Navigation moins pratique sur un site web que dans une application mobile.</p>
  </div>
  <div class="flex flex-col gap-3">
    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-brand-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg>
    <p class="text-sm text-hero-foreground/80">Difficulté à passer rapidement entre recettes, menus et liste de courses.</p>
  </div>
  <div class="flex flex-col gap-3">
    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-brand-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
    <p class="text-sm text-hero-foreground/80">Difficulté à suivre précisément l'évolution de la diversification de bébé.</p>
  </div>
  <div class="flex flex-col gap-3">
    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-brand-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"/></svg>
    <p class="text-sm text-hero-foreground/80">Difficulté à retrouver rapidement ses recettes préférées sur le site.</p>
  </div>
</div>
</SectionDark>

<CaseStudyRoleSection title="Rôle et processus">
  <CaseStudyRole
    label="Rôle"
    title="Product Design"
    icon="layout"
    image={image37}
    imageAlt="Écrans de l’application Cuisinez pour Bébé"
  >
    Notre équipe a participé à la conception de l’expérience mobile à partir de
    l’analyse des besoins utilisateurs. Et participe également à la
    structuration des parcours et réalisation de prototypes basse et haute
    fidélité, avec des itérations pour valider les choix de conception.
    <CaseStudyTags slot="tags" tags={['Figma', 'UX/UI', 'Maquettes']} />
    <CaseStudyTeam
      slot="team"
      label="Équipe sur ce rôle"
      members={[
        {
          name: 'Ivan D.',
          role: 'Lead Designer UX/UI',
          href: '/fr/equipe/ivan-dalmet',
        },
        {
          name: 'Daryl A.',
          role: 'UX/UI Designer',
          href: '/fr/equipe/daryl-avila-bonnet',
        },
        {
          name: 'Dorian D.',
          role: 'UX/UI Designer',
          href: '/fr/equipe/dorian-delorme',
        },
      ]}
    />
  </CaseStudyRole>
  <CaseStudyRole label="Rôle" title="Développement" icon="code">
    Côté technique, BearStudio a assuré la réalisation de l’application mobile
    connectée à l’infrastructure WordPress existante : développement React
    Native (Android & iOS), intégration via API REST WordPress, recherche
    Algolia, authentification et modèle freemium.
    <CaseStudyTags
      slot="tags"
      tags={['React Native', 'WordPress API', 'Algolia']}
    />
    <CaseStudyTeam
      slot="team"
      label="Équipe sur ce rôle"
      members={[{ name: 'Lead Dev', role: 'Lead Développeur Full-Stack' }]}
    />
  </CaseStudyRole>
...
<div class="not-prose grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
  <ReviewCard
    name="Marie Aixdéelle"
    date="22/03/2025"
    rating={5}
    store="app-store"
  >
    Franchement, application très utile, Sans abonnement, accès à des multiples
    recettes, au menu de la semaine, aux fiches aliments, aux guides de la
    diversification à 4, 6 et 9 mois et des articles. On a envie de cuisiner
    pour bébé !
  </ReviewCard>
  <ReviewCard name="Sophie L." date="15/02/2025" rating={5} store="google-play">
    Super application ! Les recettes sont claires et faciles à suivre. La liste
    de courses est un vrai gain de temps. Je recommande à tous les parents.
  </ReviewCard>
  <ReviewCard name="Thomas B." date="08/01/2025" rating={5} store="app-store">
    Indispensable depuis que notre fils a commencé la diversification. Les menus
    hebdomadaires nous simplifient la vie et le batch cooking est top.
  </ReviewCard>
</div>
